...
def update_adj(lookup, brackets, adj, is_reversed, front, back, direction, dummy, d, l, r):  # Time: O(K)
    ...
    for src in direction(brackets):
        ...
        if via == dst:
            ...
            w = lookup[via][not is_reversed][src] + \
                lookup[via][is_reversed][dst]
...
def find_partitions(PRG, brackets):  # Time: O(K)
    ...
    while 2*(right-left+1) <= len(brackets)+2:  # including virtual brackets we added
        ...
        left, right = find_next_bracket(
            PRG, brackets, left, -1), find_next_bracket(PRG, brackets, right, 1)
...
def build(PRG, L, R, P, pair, lookup, tree, node):  # Time: O(KlogK)
    ...
    partitions = map(lambda x: outer_l if x == -1 else (outer_r if x == len(brackets)
                                                        else brackets[x]), partition_idxs)  # replace virtual brackets with outer brackets
    children = [0]*4
    ...
    for i in partition_idxs:
        if i in (-1, len(brackets)):  # virtual brackets we added
            continue
        lookup[brackets[i]] = find_shortest_path(
            L, R, P, pair, lookup, brackets, brackets[i])  # Time: O(KlogK)
    for i in xrange(len(partition_idxs)):
        new_brackets = find_subregions(brackets, partition_idxs, i)
        if not new_brackets:
            continue
        new_outer_l, new_outer_r = find_outer_brackets(
            pair, brackets, partition_idxs, i, outer_l, outer_r)
        children[i] = len(tree)
        tree.append([new_brackets, new_outer_l, new_outer_r])


# Time: O(K * (logK)^2) for lazy build, O(QlogK) for query, run at most O(KlogK) in each depth, at most O(logK) depth
def query(PRG, L, R, P, pair, lookup, tree, node, s, e):
    # 2^d-1 >= k, d >= ceil(log(k+1))
    depth, ceil_log2_Kp1 = 0, ((len(PRG)+1)-1).bit_length()
    while True:
        depth += 1
        ...
        a, b = map(lambda x: bisect_left(partitions, x) %
                   len(partitions), (s, e))
        if s == partitions[a] or e == partitions[b] or a != b:
            break
        # same subregion without covering partition nodes, visit subregion
        node = children[a]
    # find min LCA dist
...
def emacspp():
    ...
    pair, lookup, tree = find_pair(
        PRG), [0]*K, [[range(len(PRG)), -1, len(PRG)]]
